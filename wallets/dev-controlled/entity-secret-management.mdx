...
<VideoContent
  url="https://fast.wistia.net/embed/iframe/aupti5g6vf?seo=true&videoFoam=true"
  header="Video Tutorial: Generate and Encrypt an Entity Secret"
>
...
The Entity Secret <Glossary term="Ciphertext" /> is a
<Glossary term="RSA encryption" /> token generated from your Entity Secret and
...
1. Use one of our server-side SDKs to generate the Entity Secret, encrypt it,
   and register it
2. Generate the Entity Secret yourself by using Standard Libraries or CLI tools,
   then use the SDK to encrypt it and register it
3. Generate the Entity Secret yourself by using Standard Libraries or CLI tools,
