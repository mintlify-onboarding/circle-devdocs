...
- **Blockchain Operations**:
  - After authentication, use toModularTransport to interact with the wallet
    on-chain (e.g., sending transactions or fetching balances).
...
The `eth_getUserOperationReceipt` and `eth_getUserOperationByHash` methods
scan logs from recently mined blocks and are not designed for historical data.
Requests for user operations outside the supported range may be rejected due
to block range limitations.
