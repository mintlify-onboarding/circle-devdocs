...
```ts import { english, generateMnemonic, mnemonicToAccount } from "viem/accounts";
...
```ts // Recreate the recovery account from the saved mnemonic
...
```ts await recoveryClient.executeRecovery({
...
```ts const recoveredAccount = await toCircleSmartAccount({
  client,
