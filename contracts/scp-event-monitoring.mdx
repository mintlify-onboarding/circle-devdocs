...
In this guide you'll set up real-time push notifications for specific
<Glossary term="Events" /> that occur in your smart contracts. You can then use
those events to trigger important functionality in your application.
...
1. [Step 1. Configure Your Webhook for Notifications](#step-1-configure-your-webhook-for-notifications)
2. [Step 2. Import Your Smart Contract](#step-2-import-your-smart-contract)
3. [Step 3. Create an Event Monitor](#step-3-create-an-event-monitor)
4. [Step 4. Fetch Event History](#step-4-fetch-event-history)
...
subscriber endpoint on your side. This endpoint should handle `POST` requests
over HTTPS.

For more information on setting up a webhook, refer to the
[Webhooks Quickstart](/wallets/webhook-notifications), and optionally watch the
following video about Webhook Configurations.

<VideoContent
  url="https://circle-com.wistia.com/medias/v3z7c3d2p1"
  header="Video Tutorial: Webhooks Configuration"
/>
...
`Limit to specific events` when creating your webhook in the Developer Console,
...
`POST` with the body formatted as follows:
...
For more information on importing a contract, see the
[Import a contract](/api-reference/contracts/smart-contract-platform/import-contract)
API endpoint.
...
data later. For more details, see
[Step 4. Fetch Event History](#step-4-fetch-event-history).
...
Please ensure the event signature exists if you are using an unverified contract
as Circle cannot verify event existence for unverified contracts.
